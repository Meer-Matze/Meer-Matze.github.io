...
- **必须包含 SIGCHLD 处理程序**，回收僵死（zombie）子进程的资源。因为 SIGCHLD 处理程序执行时该信号被阻塞，而 Linux 信号不排队，所以处理程序必须准备好用 `while` 循环回收多个僵死子进程
- **父子进程必须各自关闭 `connfd` 副本**。对父进程而言这尤为重要，它必须关闭已连接描述符以避免内存泄漏
- **因为套接字文件表表项的引用计数**，直到父子进程的 `connfd` 都关闭了，到客户端的连接才会终止

### 12.1.2 进程的优劣

对于父、子进程间共享状态信息，进程有一个非常清晰的模型：共享文件表，但不共享用户地址空间。进程有独立的地址空间既是优点也是缺点：一个进程不可能不小心覆盖另一个进程的虚拟内存，消除了许多令人迷惑的错误，这是明显的优点。

另一方面，独立的地址空间使得进程共享状态信息变得更加困难，必须使用显式的 **IPC**（进程间通信）机制。基于进程的设计的另一个缺点是通常比较慢，因为进程控制和 IPC 的开销很高。

> [!note] Unix IPC
> 本书已经介绍过几种 IPC：第八章的 `waitpid` 和信号、第十一章的套接字接口。Unix IPC 通常指同一主机上进程间通信的技术，包括管道、FIFO、System V 共享内存和 System V 信号量。这些机制超出本章范围，Kerrisk 的著作是参考资料。
...
`select` 是一个复杂的函数，有许多使用场景。本节只讨论最常见的一种：等待一组描述符准备好读。这时后三个参数固定传 `NULL`（不关心写就绪、异常就绪，也不设置超时），调用形式简化为：
...
> [!note] 简化调用不等于简化原型
> `select` 的完整签名有四个描述符集合参数和一个超时参数；本节只用到第一个描述符集合。后面出现的几个函数（如 `sem_init`）也会有类似情况——某个参数在本书的用法中总是固定值，但函数本身的签名并不会因此变化。

`select` 函数处理类型为 `fd_set` 的集合，也叫做描述符集合。逻辑上，我们将描述符集合看成一个大小为 $n$ 的位向量（第二章介绍过）：$b_{n-1},\cdots,b_1,b_0$。当且仅当 $b_k=1$，描述符 $k$ 才是描述符集合的一个元素。只允许对描述符集合做三件事：分配它们、把一个此类型变量赋值给另一个、用 `FD_ZERO`/`FD_SET`/`FD_CLR`/`FD_ISSET` 宏修改和检查它们。

针对我们的目的，`select` 的两个关键输入是**读集合**（`readfds`）和该集合的基数 `n`（实际上是任何描述符集合的最大基数）。`select` 会一直阻塞，直到读集合中至少有一个描述符准备好可以读——当且仅当一个从该描述符读一字节的请求不会阻塞时，描述符 $k$ 才**准备好可以读**。`select` 有一个副作用：它会修改参数指向的 `fd_set`，指明读集合的一个子集，称为**准备好集合**（ready set）。返回值指明准备好集合的基数。由于这个副作用，每次调用 `select` 前都必须重新初始化读集合。

理解 `select` 的最好办法是研究一个具体例子——用它实现一个能同时接受用户命令的迭代 echo 服务器：
...
不调用 `accept` 等待连接，而是调用 `select`，阻塞直到监听描述符或标准输入准备好可读。一旦 `select` 返回，用 `FD_ISSET` 确定哪个描述符准备好了：标准输入准备好就调用 `command` 读取解析命令；监听描述符准备好就 `accept` 并调用 `echo`。

这个程序仍留有问题：一旦连接到某个客户端，就会连续回送输入行，直到该客户端关闭连接——这期间键入命令得不到响应。更好的方法是更细粒度的多路复用，服务器每次循环至多回送一个文本行（见下一节）。

### 12.2.1 基于 I/O 多路复用的并发事件驱动服务器

I/O 多路复用可以用做并发**事件驱动**（event-driven）程序的基础：某些事件导致流向前推进。一般思路是将逻辑流模型化为**状态机**（state machine）——一组**状态**（state）、**输入事件**（input event）和**转移**（transition），转移把（状态，输入事件）对映射到新状态。同一输入和输出状态之间的转移叫**自循环**（self-loop）。

对每个新客户端 $k$，服务器创建一个新的状态机 $s_k$，与已连接描述符关联：状态是"等待描述符 $d_k$ 准备好可读"，输入事件是"描述符 $d_k$ 准备好可读"，转移是"从描述符 $d_k$ 读一个文本行"。服务器用 `select` 检测输入事件，当已连接描述符准备好可读时，就为相应状态机执行转移。

一个 `pool` 结构维护活动客户端的集合：
...
| 运行在单一进程上下文中，流之间共享数据容易；可以像调试顺序程序一样用 GDB 调试 | 不能充分利用多核处理器                                                               |
| 不需要进程上下文切换来调度新流，通常更高效                                    | 并发粒度越小，复杂性越高——若流读取部分文本行时其他流无法前进，恶意客户端可利用这一点 |

并发粒度是每个逻辑流在一个时间片中执行的指令数量。示例中粒度是读取一整行所需的指令数量。基于进程的设计能自动处理部分文本行，事件驱动设计则需要额外修改才能应对。

## 12.3 基于线程的并发编程

前两种方法各有取舍：进程由内核自动调度但地址空间私有，共享数据困难；I/O 多路复用共享地址空间但需要自己显式调度流。**线程**（thread）是这两种方法的混合。

线程就是运行在进程上下文中的逻辑流，由内核自动调度。每个线程都有自己的**线程上下文**（thread context）——唯一的整数**线程 ID**（TID）、栈、栈指针、程序计数器、通用寄存器和条件码。同一进程里的所有线程共享该进程的整个虚拟地址空间，包括代码、数据、堆、共享库和打开的文件。

### 12.3.1 线程执行模型

每个进程开始时都是单一线程，称为**主线程**（main thread）。主线程创建**对等线程**（peer thread）后，两个线程并发运行；因为慢速系统调用（如 `read`、`sleep`）或系统间隔计时器中断，控制通过上下文切换在线程间传递。

线程执行和进程有几点重要区别：线程上下文比进程上下文小得多，因此线程上下文切换比进程上下文切换快得多；线程不像进程那样按严格的父子层次组织，而是组成一个**对等线程池**——主线程和其他线程的区别仅在于它总是最先运行的线程。因此一个线程可以杀死它的任何对等线程，或等待任意对等线程终止，每个对等线程都能读写相同的共享数据。
...
线程的代码和本地数据封装在**线程例程**（thread routine）中，以通用指针为输入并返回通用指针——传多个参数需放入结构体，返回多个值同理。主线程调用 `pthread_create` 创建对等线程；返回后两个线程同时运行，`tid` 包含新线程 ID；`pthread_join` 等待对等线程终止；`exit` 终止当时运行在这个进程中的所有线程。
...
`pthread_create` 创建一个新线程，带输入变量 `arg`，在新线程上下文中运行线程例程 `f`。`attr` 可改变新线程的默认属性，本章始终传 `NULL`。返回时 `tid` 包含新线程 ID；新线程也可通过 `pthread_self` 获得自己的线程 ID。
...
- **隐式终止**：顶层线程例程返回时
- **显式终止**：调用 `pthread_exit`（若主线程调用它，主线程终止但进程继续运行，直到其他线程也终止）
- 某个对等线程调用 Linux 的 `exit` 函数，终止进程及所有相关线程
- 另一个线程调用 `pthread_cancel` 请求终止目标线程（是否立即终止取决于其取消状态和取消点）

```c
#include <pthread.h>
void pthread_exit(void *thread_return);   /* 从不返回 */
int pthread_cancel(pthread_t tid);        /* 成功返回 0，出错为非零 */
```
...
> 和 Linux 的 `wait` 不同，`pthread_join` 只能等待一个指定线程终止。Pthreads 没有等待任意线程终止的 `pthread_wait` 函数，这会让需要等待任意工作线程的程序更复杂——Stevens 认为这是规范中的一个缺陷。

### 12.3.6 分离线程

线程在任何时刻是**可结合的**（joinable）或**分离的**（detached）。可结合线程能被其他线程收回和杀死，回收前内存资源不释放；分离线程不能被回收或杀死，终止时系统自动释放其内存资源。
...
线程可用 `pthread_self()` 作参数分离自己。实践中很有理由使用分离线程：例如高性能 Web 服务器为每个连接创建一个对等线程，服务器不需要（也不愿意）显式等待每个线程终止——每个对等线程应在开始处理请求前就分离自己，这样终止后能立即回收内存资源。
...
`once_control` 是全局或静态变量，初始化为 `PTHREAD_ONCE_INIT`。第一次以某个 `once_control` 调用 `pthread_once` 时会执行 `init_routine`；后续以同一 `once_control` 调用不做任何事。这在需要动态初始化多个线程共享的全局变量时很有用（见 §12.5.5）。
...
整体结构类似基于进程的设计，但有一个微妙问题：如何把已连接描述符传给对等线程？直接传指向该描述符的指针（`&connfd`）会在对等线程的赋值语句和主线程的下一次 `accept` 之间引入**竞争**（race）——若赋值发生在下一次 `accept` 之后，对等线程读到的就是下一次连接的描述符值，导致两个线程操作同一个描述符。因此必须把 `accept` 返回的每个描述符分配到独立的动态内存块（`Malloc(sizeof(int))`），见上面代码第 20~21 行。
...
理解一个变量是否共享，需要回答三个问题：线程的基础内存模型是什么？变量实例如何映射到内存？有多少线程引用这些实例？——一个变量是**共享的**，当且仅当多个线程引用它的某个实例。
...
| 存储类型                    | 定义位置 | 运行时实例数                            |
| --------------------------- | -------- | --------------------------------------- |
| 全局变量                    | 函数外部 | 读/写区域中只有一个实例，任何线程可引用 |
| 本地自动变量（无 `static`） | 函数内部 | 每个线程的栈各有自己的实例              |
| 本地静态变量（带 `static`） | 函数内部 | 读/写区域中只有一个实例                 |

示例中 `myid` 在两个对等线程的栈中各有一个实例（`myid.p0`、`myid.p1`）；`cnt` 是本地静态变量，由两个对等线程共同读写。

### 12.4.3 共享变量

变量 $v$ 是共享的，当且仅当它的一个实例被一个以上的线程引用。示例中 `cnt` 只有一个运行时实例、被两个对等线程引用，是共享的；`myid` 的两个实例各只被一个线程引用，不是共享的。值得注意的是，像 `msgs` 这样的本地自动变量也能被共享（通过 `ptr` 间接引用）。
...
把 `cnt++` 循环拆成五个部分：$H_i$（循环头指令块）、$L_i$（加载 `cnt` 到寄存器）、$U_i$（更新寄存器）、$S_i$（存回 `cnt`）、$T_i$（循环尾指令块）。$H_i$、$T_i$ 只操作本地栈变量，$L_i$、$U_i$、$S_i$ 操作共享变量。

两个线程的指令以某种全序交叉执行。**关键点：你没有办法预测操作系统会为线程选择一个正确的顺序。**若线程 2 在第 5 步加载 `cnt`（发生在线程 1 加载 `cnt` 之后、存储更新值之前），两个线程最终都会存回值 1，而不是期望的 2。
...
- $H_1,L_1,U_1,S_1,H_2,L_2,U_2,S_2,T_2,T_1$：**安全**
- $H_2,L_2,H_1,L_1,U_1,S_1,T_1,U_2,S_2,T_2$：**不安全**
- $H_1,H_2,L_2,U_2,S_2,L_1,U_1,S_1,T_1,T_2$：**安全**
