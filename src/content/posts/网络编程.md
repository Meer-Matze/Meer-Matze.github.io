...
> [!important] 本章目标
> 学完本章，你将能编写一个为真实浏览器提供静态和动态内容的 Web 服务器。
...
1. 客户端向服务器发送请求，发起事务
2. 服务器收到请求，解释它，并操作资源
3. 服务器给客户端发送响应，并等待下一个请求
4. 客户端收到响应并处理它

> [!note] 客户端和服务器是进程
> 客户端和服务器是进程，不是机器或主机。一台主机可以同时运行多个客户端和服务器。事务可以在同一台或不同主机上进行。

> [!note] 与数据库事务的区别
> 客户端—服务器事务不是数据库事务。它没有原子性等数据库事务特性。这里的事务只是客户端和服务器执行的一系列步骤。
