...
updated: 2026-09-19
description: "客户端-服务器模型、网络、套接字接口与 Web 服务器"
image: ""
tags: [计算机原理]
category: "计算机基础"
series: "CSAPP"
...
> [!summary] 本章内容
> 网络应用依赖很多已学过的概念：进程、信号、字节顺序、内存映射、动态内存分配。具体来说，本章涉及以下内容：
>
> - 客户端—服务器模型
>   - 服务器管理资源，为客户端提供服务。基本操作是事务，由客户端请求和服务器响应组成。
> - 客户端和服务器通过因特网这个全球网络来通信
>   - 从程序员角度，因特网是全球范围的主机集合：每个因特网主机有唯一 32 位名字，即 IP 地址；IP 地址集合映射为因特网域名集合；不同主机上的进程能通过连接通信。
> - 客户端和服务器通过套接字接口建立连接
>   - 套接字是连接端点，以文件描述符形式提供给应用程序，客户端和服务器通过读写描述符通信。
> - Web 服务器用 HTTP 协议与客户端通信
>   - 静态内容从服务器磁盘取得文件并返回
>   - 动态内容在子进程上下文运行程序并返回输出
>     - CGI 标准管理参数传递、信息传递和输出发送。
>
...
对 Web 客户端和服务器而言，**内容**是与 [**MIME**](iana.org/assignments/media-types)（Multipurpose Internet Mail Extensions，多用途网际邮件扩充协议）类型相关的字节序列：
...
每条返回内容都与服务器管理的某个文件关联，每个文件都有唯一名字 **URL**（Universal Resource Locator，通用资源定位符）。

例如 URL `http://www.google.com:80/index.html`表示主机 `www.google.com` 上名为 `/index.html` 的 HTML 文件，由监听端口 80 的 Web 服务器管理。端口号是可选的，默认为知名 HTTP 端口 80。

可执行文件的 URL 可在文件名后包括参数，"?" 分隔文件名和参数，每个参数用 "&" 分隔：`http://bluefish.ics.cs.cmu.edu:8000/cgi-bin/adder?15000&213` 标识可执行文件 `/cgi-bin/adder`，并附带两个参数 15000 和 213。

事务中，客户端使用前缀（`http://www.google.com:80`）决定联系哪类服务器、服务器在哪、监听端口；服务器使用后缀（`/index.html`）发现文件，确定请求静态还是动态内容。

- 确定静态还是动态内容没有标准规则，每个服务器有自己的规则，经典方法是确定一组目录（如 `cgi-bin`），所有可执行文件必须存放其中。
- 后缀中最开始的 `/` 不表示 Linux 根目录，而是被请求内容类型的主目录，由服务器定义。
- 最小 URL 后缀是 `/`，所有服务器扩展为默认主页（如 `/index.html`）。
  - 这解释了为什么简单地在浏览器中键入一个域名就可以取出一个网站的主页。浏览器在 URL 后添加缺失的 `/`，并将之传递给服务器，服务器又把 `/` 扩展到某个默认的文件名。
...
> [!note] 现在 www.aol.com 已经使用 HTTPS（HTTP over SSL）加密传输，TELNET 无法连接
...
HTTP 支持多种方法：GET、POST、OPTIONS、HEAD、PUT、DELETE、TRACE，本章将只讨论 GET。GET 指导服务器生成并返回 **URI**（Uniform Resource Identifier，统一资源标识符）标识的内容，URI 是 URL 的后缀，包括文件名和可选参数。

> [!note] URI 与代理
> 只有当浏览器请求内容时 URI 才是 URL 的后缀。如果代理服务器请求内容，URI 必须是完整 URL。

`version` 字段表明请求遵循的 HTTP 版本：

- HTTP/1.0 是 1996 年以来的老版本
- HTTP/1.1 定义附加报头，支持缓冲、安全等高级特性，支持**持久连接**（persistent connection）上执行多个事务
- HTTP/2 是 2015 年的最新版本，支持二进制传输、流、头压缩等高级特性
- HTTP/3 是 2020 年的最新版本，基于 QUIC 协议，支持多路复用、连接迁移等高级特性

截止 2024 年，根据 cloudflare 的统计，HTTP/2 占比约 50%，HTTP/3 占比约 20%，但 HTTP/1.1 仍然被兼容使用。
...
唯一需要关注的报头是 Host 报头：在 HTTP/1.1 请求中需要，HTTP/1.0 不需要

**代理缓存**（proxy cache）使用Host 报头，代理缓存是浏览器和**原始服务器**（origin server）的中介，类似处理器和内存之间的缓存，客户端和原始服务器之间可有多个代理，即**代理链**（proxy chain），Host 报头指示原始服务器域名，使代理判断是否可在本地缓存拥有副本。

#### 2. HTTP 响应

HTTP 响应由**响应行**（response line）组成，后跟零个或更多**响应报头**（response header），再跟空行，再跟**响应主体**（response body）。响应行形式为
...
version 字段描述的是响应所遵循的 HTTP 版本。**状态码**（status-code）是一个 3 位的正整数，指明对请求的处理。状态消息（status message）给出与错误代码等价的英文描述。常见状态码：

| 状态代码 | 状态消息                   | 描述                               |
| -------- | -------------------------- | ---------------------------------- |
| 200      | OK                         | 处理请求无误                       |
| 301      | Moved Permanently          | 内容已移动到 location 头指明的主机 |
...
GET 请求的参数在 URI 中传递："?" 分隔文件名和参数，每个参数用 "&" 分隔，参数中不允许空格，用字符串 "%20" 表示，其他特殊字符有相似编码（比如所有中文字符）。
...
服务器收到请求后调用 `fork` 创建子进程，调用 `execve` 在子进程上下文执行对应程序，子进程把 CGI 环境变量 `QUERY_STRING` 设置为参数字符串，CGI 程序运行时用 `getenv` 函数获取该变量。

> [!note] CGI 程序
> 由于生成动态内容的程序符合 CGI 标准，称为 CGI 程序。而且，由于许多 CGI 程序是用 Perl、Python、PHP 等脚本语言编写的，也被称为 CGI 脚本。
...
```c title="netp/tiny/agi-bin/adder.c"
#include "csapp.h"
...
int main(void) {
    ...
    snprintf(content, sizeof(content), "<p>%d + %d = %d\r\n</p>", n1, n2, n1 + n2);
...
main 逻辑：调用 `open_listenfd` 打开监听套接字；执行无限服务器循环；接受连接请求，执行事务`doit`，关闭连接。
...
```mermaid
flowchart TD
    A@{ shape: sm-circ, label: "Small start" } --> B[read_requesthdrs]
    B -->|not GET| I[clienterror 501]
    B --> C[parse_uri]
    C --> E{file exists?}
    E -->|yes| D{is_static?}
    D -->|yes| H{is regular file\nand readable?}

    H -->|yes| J[serve_static]
    D -->|no| N{is regular file\nand executable?}
    E -->|no| G[clienterror 404]
    N & H -->|no| O[clienterror 403]
    N -->|yes| P[serve_dynamic]
```
...
> [!warning] 内存重叠
> 这里 `sprintf(body, "%s...", body)` 中，`body` 同时作为目标字符串和参数使用，属于未定义行为。但CSAPP 书中使用了这种写法，请勿模仿。
...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
    ...
    if (!strstr(uri, "cgi-bin")) {  // 静态内容
        ...
        ptr = index(uri, '?');
        if (ptr) {
            strcpy(cgiargs, ptr + 1);   // 将"?" 后的所有字符复制到 cgiargs
            *ptr = '\0';
        }
        else
            strcpy(cgiargs, "");
...
- 静态内容清除 CGI 参数字符串，把 URI 转换为相对路径名
  - URI 以 `/` 结尾时加默认文件名
- 动态内容抽取所有 CGI 参数，把 URI 剩余部分转换为相对文件名。
...
void serve_static(int fd, char *filename, int filesize)
{
    ...
    /* 发送响应主体 */
    srcfd = Open(filename, O_RDONLY, 0);    // 以读方式打开 filename，并获得它的描述符
    srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0); // 将文件 srcfd 的前 filesize 个字节映射到一个从地址 srcp 开始的私有只读虚拟内存区域
    Close(srcfd);   // 一旦将文件映射到内存，就不再需要它的描述符了
    Rio_writen(fd, srcp, filesize); // 写入 fd 来发送响应主体
...
void serve_dynamic(int fd, char *filename, char *cgiargs)
{
    ...
    if (Fork() == 0) {  // 子进程完成 CGI 任务
        // 真正的服务器会在此设置其他所有 CGI 环境变量
        setenv("QUERY_STRING", cgiargs, 1);
        Dup2(fd, STDOUT_FILENO);    // 重定向标准输出到已连接描述符
...
因为 CGI 程序运行在子进程的上下文中，它能够访问所有在调用 `execve` 函数之前就存在的打开文件和环境变量。因此，重定向标准输出后，CGI 程序写到标准输出上的任何东西都将直接送到客户端进程，不会受到任何来自父进程的干涉。
