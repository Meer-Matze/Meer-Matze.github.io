...
例：如图所示的两级放大电路，已知$R_1=15\text{k}\Omega, R_2=R_3=5\text{k}\Omega, R_4=2.3\text{k}\Omega, R_5=100\text{k}\Omega, R_6=R_L=5\text{k}\Omega;V_{CC}=12\text{V}, \beta_1 = \beta_2 = \beta = 150, r_{be1} = 4\text{k}\Omega, r_{be2} = 2.2\text{k}\Omega, U_{BEQ1} = U_{BEQ2} = 0.7\text{V}$，试求：
...
|:---:|:---:|:---:|:---:|:---:|:---:|
| **电压串联** | 电压 | 电压差 | 输出电压 | $A_{uf}=U_o/U_i$ | 同相比例运放 |
| **电流串联** | 电压 | 电压差 | 输出电流 | $A_{uf}=U_o/U_i$（$U_o$对应$I_o$） | 带$R_e$的共射 |
| **电压并联** | 电流 | 电流差 | 输出电压 | $A_{uf}=U_o/I_i$（互阻） | 反相比例运放 |
| **电流并联** | 电流 | 电流差 | 输出电流 | $A_{uf}=I_o/I_i$（电流增益） | 共射+反馈电阻 |
...
- **四种组态判断**：
  1. **电压/电流**：输出端短路（$u_o=0$）时
      ...
      - 仍存在 → **电流反馈**。
  2. **串联/并联**：
      - 反馈量与输入量在输入端**串联**（电压求和）→ **串联反馈**（增大$R_i$）
      - 反馈量与输入量在输入端**并联**（电流求和）→ **并联反馈**（减小$R_i$）
...
| 组态 | 反馈系数 $\dot{F}$ | $A_f \approx 1/\dot{F}$ |
|:---:|:---:|:---|
| **电压串联** | $\dot{F}_{uu} = \dfrac{\dot{U}_f}{\dot{U}_o}$ | $A_{uuf} = \dfrac{1}{\dot{F}_{uu}}$ |
| **电流串联** | $\dot{F}_{ui} = \dfrac{\dot{U}_f}{\dot{I}_o}$ | $A_{uf} = \dfrac{\dot{U}_o}{\dot{U}_i} \approx \dfrac{\dot{I}_o R_L}{\dot{U}_i} = \dfrac{R_L}{\dot{F}_{ui}}$ |
| **电压并联** | $\dot{F}_{iu} = \dfrac{\dot{I}_f}{\dot{U}_o}$ | $A_{usf} = \dfrac{\dot{U}_o}{\dot{I}_i R_s} \approx \dfrac{1}{\dot{F}_{iu} R_s}$ |
| **电流并联** | $\dot{F}_{ii} = \dfrac{\dot{I}_f}{\dot{I}_o}$ | $A_{isf} = \dfrac{\dot{I}_o}{\dot{I}_i} \approx \dfrac{1}{\dot{F}_{ii}}$ |
